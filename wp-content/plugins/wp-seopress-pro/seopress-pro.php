...
//Define
///////////////////////////////////////////////////////////////////////////////////////////////////
define('SEOPRESS_PRO_VERSION', '7.3.1');
...
if (file_exists(__DIR__ . '/vendor/autoload.php') && file_exists(WP_PLUGIN_DIR . '/wp-seopress/seopress-autoload.php')) {
    ...
    if ('7.3.1' !== $versionFree && version_compare($versionFree, '4.5.1', '<=')) {
        return;
    }
...
function seopress_pro_admin_scripts(){
    ...
    wp_enqueue_script('seopress-pro-ai-js', plugins_url('assets/js/seopress-pro-ai' . $prefix . '.js', __FILE__), ['jquery'], SEOPRESS_PRO_VERSION, true);
...
function seopress_pro_add_admin_options_scripts($hook) {
    ...
    if ('index.php' == $pagenow || (isset($_GET['page']) && 'seopress-option' === $_GET['page'])) {
        if (seopress_pro_get_service('GoogleAnalyticsWidgetsOptionPro')->getGA4DashboardWidget() !== '1') {
            ...
            //GA API
            wp_enqueue_script('seopress-pro-ga-embed', plugins_url('assets/js/chart.bundle' . $prefix . '.js', __FILE__), [], SEOPRESS_PRO_VERSION);

            wp_enqueue_script('seopress-pro-ga', plugins_url('assets/js/seopress-pro-ga' . $prefix . '.js', __FILE__), ['jquery', 'jquery-ui-tabs'], SEOPRESS_PRO_VERSION);
    ...
    if ('index.php' == $pagenow || (isset($_GET['page']) && 'seopress-option' === $_GET['page'])) {
        if (seopress_pro_get_service('GoogleAnalyticsWidgetsOptionPro')->getMatomoDashboardWidget() !== '1') {
            ...
            wp_enqueue_script('seopress-pro-matomo', plugins_url('assets/js/seopress-pro-matomo' . $prefix . '.js', __FILE__), ['jquery', 'jquery-ui-tabs'], SEOPRESS_PRO_VERSION);
    ...
    if ('widgets.php' == $pagenow) {
        wp_enqueue_script('seopress-pro-lb-widget', plugins_url('assets/js/seopress-pro-lb-widget' . $prefix . '.js', __FILE__), ['jquery', 'jquery-ui-tabs'], SEOPRESS_PRO_VERSION);
    ...
    if (isset($_GET['page']) && ('seopress-pro-page' == $_GET['page'])) {
        //Admin tabs
        wp_enqueue_script('seopress-pro-admin-tabs-js', plugins_url('assets/js/seopress-pro-tabs' . $prefix . '.js', __FILE__), ['jquery-ui-tabs'], SEOPRESS_PRO_VERSION);

        //Search Console
        wp_enqueue_script('seopress-pro-search-console-js', plugins_url('assets/js/seopress-pro-search-console' . $prefix . '.js', __FILE__), ['jquery'], SEOPRESS_PRO_VERSION);
        ...
        wp_localize_script('seopress-pro-ai-js', 'seopressAjaxAICheckLicense', $seopress_ai_check_license_key);
    }
    ...
    if (isset($_GET['page']) && ('seopress-bot-batch' == $_GET['page'])) {
        wp_enqueue_script('seopress-bot-admin-tabs-js', plugins_url('assets/js/seopress-bot-tabs' . $prefix . '.js', __FILE__), ['jquery-ui-tabs'], SEOPRESS_PRO_VERSION);
...
<?php
        ...
        if ( version_compare(SEOPRESS_PRO_VERSION, '5.4', '<')) {
            ...
            <div class="<?php echo $class; ?>">
                <p>
                    <?php printf(__('A new <strong>SEOPress PRO</strong> update (v%1$s) is available (current installed version %2$s). <br>Please update now to get new features and prevent any issues.', 'wp-seopress-pro'), '<code>7.1</code>', '<code>'.SEOPRESS_PRO_VERSION.'</code>'); ?>
                </p>
                <p>
