...
class Completions {
	...
	public function generateImgAltText($post_id, $action = '', $language = 'en_US') {
		...
		//Prompt for alt text
		$prompt_alt_text = sprintf(__('Write in less than 10 words an alternative text to improve accessibility and SEO, in this language %s.', 'wp-seopress-pro'), esc_attr($language));

		$msg   = apply_filters( 'seopress_ai_openai_alt_text', $prompt_alt_text, $post_id );

		$body = [
			'model'    => $this->getOpenAIModel() === 'gpt-4' ? 'gpt-4-vision-preview' : $this->getOpenAIModel(),
			'temperature' => 1,
            'messages' => [
				[
					'role' => 'user',
        ...
        if(!empty($alt_text)){
            update_post_meta($post_id, '_wp_attachment_image_alt', apply_filters('seopress_update_alt', $alt_text, $post_id));
        }
